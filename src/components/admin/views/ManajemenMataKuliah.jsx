...
export default function ManajemenMataKuliah({ onActionSuccess, logActivity }) {
    ...
    const handleTambahMK = async (e) => {
        ...
        try {
            ...
            setHari('Senin'); setJamMulai(''); setJamSelesai(''); setRuangan('');
            onActionSuccess();
        } catch (error) {
    ...
    return (
        // PERBAIKAN: class max-w-2xl mx-auto dihapus agar form melebar
            ...
            <form onSubmit={handleTambahMK} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value={kodeMK} onChange={e => setKodeMK(e.target.value)} required placeholder="Contoh: SI101" className="w-full p-3 border border-gray-300 rounded-lg"/>
                    </div>
                    <div>
                        <label className='block text-sm font-medium text-gray-700 mb-1'>Nama Mata Kuliah</label>
                        <input type="text" value={namaMK} onChange={e => setNamaMK(e.target.value)} required placeholder="Contoh: Pemrograman Web" className="w-full p-3 border border-gray-300 rounded-lg"/>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" value={semesterMK} onChange={e => setSemesterMK(e.target.value)} required placeholder="Untuk semester ke-" className="w-full p-3 border border-gray-300 rounded-lg"/>
                    </div>
                </div>

                <div className="border-t pt-6">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Atur Jadwal & Ruangan</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" value={ruangan} onChange={e => setRuangan(e.target.value)} required placeholder="Contoh: Lab Komputer 1" className="w-full p-3 border border-gray-300 rounded-lg"/>
                        </div>
                    </div>
                ...
                <div className="pt-4">
                    <button 
                        type="submit" 
                        disabled={isLoading} 
                        className="w-full flex items-center justify-center gap-2 bg-purple-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-purple-700 transition-colors disabled:bg-gray-400"
                    >
                        {isLoading ? (
                             <>
                            ...
                            <><Plus className="w-5 h-5" /><span>Tambah Mata Kuliah</span></>
                        )}
                    </button>
                </div>
            </form>
