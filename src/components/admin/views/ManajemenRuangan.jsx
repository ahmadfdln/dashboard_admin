...
export default function ManajemenRuangan({ onActionSuccess, logActivity }) {
    ...
    const handleTambahRuangan = async (e) => {
        ...
        if (isNaN(radiusValue) || radiusValue < 10) {
            return toast.warn("Radius minimal adalah 10 meter.");
        }
        ...
        try {
            ...
            setKodeRuangan(''); setNamaGedung(''); setLatitude(''); setLongitude(''); setRadius(100);
    ...
    return (
        ...
        <form onSubmit={handleTambahRuangan} className="space-y-4 bg-white p-8 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-center text-gray-500 pt-1">Atau klik langsung pada peta untuk memilih lokasi.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                <div>
                    <label className='block text-sm font-medium text-gray-700'>Latitude</label>
                    <input type="number" step="any" value={latitude} onChange={e => setLatitude(e.target.value)} required placeholder="Otomatis terisi..." className="mt-1 w-full p-2 border border-gray-300 rounded-md bg-gray-50"/>
                </div>
                <div>
                    <label className='block text-sm font-medium text-gray-700'>Longitude</label>
                    <input type="number" step="any" value={longitude} onChange={e => setLongitude(e.target.value)} required placeholder="Otomatis terisi..." className="mt-1 w-full p-2 border border-gray-300 rounded-md bg-gray-50"/>
                </div>
            </div>
            
            <div className='pt-2'>
                <label className='block text-sm font-medium text-gray-700'>Radius Absensi (meter): <span className="font-bold text-orange-600">{radius}m</span></label>
                <input type="range" min="10" max="200" step="1" value={radius} onChange={e => setRadius(e.target.value)} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer mt-2"/>
                 <input type="number" min="10" max="200" step="1" value={radius} onChange={e => setRadius(e.target.value)} className="mt-2 w-full p-2 border border-gray-300 rounded-md"/>
            </div>
